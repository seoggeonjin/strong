...
public class ph {
    public static final int MAX_MOVABLE_BLOCKS = 2147483647;
    ...
    public ph(World world, BlockPos pos, Direction dir, boolean retracted) {
        this.world = world;
        this.posFrom = pos;
    ...
    private boolean tryMove(BlockPos pos, Direction dir) {
        ...
        while (isBlockSticky(blockState)) {
            ...
            if (++i + this.movedBlocks.size() <= MAX_MOVABLE_BLOCKS) continue;
            return false;
        }
        int j = 0;
        for (k = i - 1; k >= 0; --k) {
